...
import {
  TokenPageProvider,
  useTokenPageContext,
} from "@/pages/hooks/useTokenPageContext";
import { TokenBalanceProvider } from "@/pages/hooks/useTokenBalanceContext";
import {
  Card,
  CardContent,
  CardHeader,
...
function TokenPageBody() {
  ...
  const { setVisible, isDesktop } = useTokenPageContext();
  ...
  const tokenAddressForBalance = details?.address ?? tokenId ?? "";
  const tokenDecimalsForBalance = Number(details?.decimals ?? 0);
  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-6 md:p-12 space-y-6">
      ...
      <TokenBalanceProvider
        tokenAddress={tokenAddressForBalance}
        tokenDecimals={tokenDecimalsForBalance}
        nativeDecimals={9}
      ...
      >
        ...
        {details && isDesktop && (
          <InstantTrading token={details} pairs={pairs} />
        )}
      </TokenBalanceProvider>
    </main>
  );
